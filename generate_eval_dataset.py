...
outfolder = r'E:/Waveformer-main/my_dataset/mixmic_eval'
...
num_samples = 1000
eval_data = []
print(f"开始生成{num_samples}个评估样本...")
for i in range(num_samples):
    if i % 100 == 0:
        print(f"进度: {i}/{num_samples}")
    sc = scaper.Scaper(duration, fg_path, bg_path)
    sc.sr = sr
    temp_bg_path = os.path.join(bg_path, 'UrbanSound8K', f'temp_bg_eval_{i}.wav')
...
print(f"评估数据集生成完成！共生成{num_samples}个样本")
csv_data = []
...
csv_file = os.path.join(outfolder, 'eval_dataset_info.csv')
pd.DataFrame(csv_data).to_csv(csv_file, index=False, encoding='utf-8-sig')
json_file = os.path.join(outfolder, 'eval_dataset_info.json')
