...
if __name__ == "__main__":
    ...
    for jams_file in jams_files:
        ...
        if fg_spec is not None and bg_spec is not None:
            rule_counter[(fg_spec, bg_spec)] += 1

    print(f"\nUnique (fg_spec, bg_spec) rule combinations: {len(rule_counter)}\n")
    for i, ((fg, bg), count) in enumerate(rule_counter.items()):
        print(f"Rule #{i+1}:")
        print(f"  Foreground (fg_spec) 前景事件数: {fg.count('foreground')}")
