...
def save_graph(train_metrics, test_metrics, save_dir):
    # 使用正确的指标名称
    metric_names = ['signal_noise_ratio', 'scale_invariant_signal_noise_ratio']
    results = {'train_loss': train_metrics['loss'],
               'test_loss' : test_metrics['loss']}

    for metric_name in metric_names:
        if metric_name in train_metrics:
            results["train_"+metric_name] = train_metrics[metric_name]
        if metric_name in test_metrics:
            results["test_"+metric_name] = test_metrics[metric_name]
    ...
    for i, metric_name in enumerate(metric_names):
        if metric_name in train_metrics and metric_name in test_metrics:
            axs[i+1].plot(x, train_metrics[metric_name], label='train')
            axs[i+1].plot(x, test_metrics[metric_name], label='test')
            axs[i+1].set(xlabel='Epoch')
            axs[i+1].set_title(metric_name, fontweight='bold')
            axs[i+1].legend()
