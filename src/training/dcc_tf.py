...
def loss(pred, tgt):
    # 检查数据是否有效
    if (tgt == 0).all() or torch.isnan(pred).any() or torch.isnan(tgt).any():
        # 如果数据无效，只使用MSE损失
        mse_loss = F.mse_loss(pred, tgt)
        return mse_loss
    
    # 使用MSE损失作为主要损失，SNR作为辅助
    mse_loss = F.mse_loss(pred, tgt)
    
    try:
        snr_val = snr(pred, tgt).mean()
        si_snr_val = si_snr(pred, tgt).mean()
        
        # 限制SNR值在合理范围内
        snr_val = torch.clamp(snr_val, -20, 20)
        si_snr_val = torch.clamp(si_snr_val, -20, 20)
        
        # 组合损失：主要使用MSE，辅助使用SNR（SNR越高损失越低）
        # SNR是负值表示质量差，正值表示质量好
        # 我们希望SNR高时损失低，所以减去SNR，但权重要小
        loss_val = mse_loss - 0.01 * snr_val - 0.005 * si_snr_val
    except:
        # 如果SNR计算失败，只使用MSE损失
        loss_val = mse_loss
...
def metrics(mixed, output, gt):
    ...
    def metric_i(metric, src, pred, tgt):
        ...
        for s, t, p in zip(src, tgt, pred):
            # 检查数据是否有效
            if (t == 0).all() or (s == 0).all() or torch.isnan(p).any() or torch.isnan(t).any() or torch.isnan(s).any():
                _vals.append(0.0)  # 如果数据无效，返回0
                continue
            # 修复：计算模型输出相对于混合音频的改善程度
            # 改善程度 = 模型输出SNR - 混合音频SNR
            model_snr = metric(p, t).cpu().item()
            mixed_snr = metric(s, t).cpu().item()
            # 检查SNR值是否有效
            if torch.isnan(torch.tensor(model_snr)) or torch.isnan(torch.tensor(mixed_snr)):
                _vals.append(0.0)
                continue
            # 限制改善程度在合理范围内，避免异常值
            improvement = model_snr - mixed_snr
            metric_val = torch.clamp(torch.tensor(improvement), -20, 20).item()
            _vals.append(metric_val)
        # 确保至少返回一个值，避免TensorBoard错误
        if len(_vals) == 0:
            _vals = [0.0]
        
        return _vals

    # 使用完整的指标名称，与配置文件中的 base_metric 保持一致
    metrics['signal_noise_ratio'] = metric_i(snr,
                                          mixed[:, :gt.shape[1], :],
                                          output,
                                          gt)
    metrics['scale_invariant_signal_noise_ratio'] = metric_i(si_snr,
                                          mixed[:, :gt.shape[1], :],
                                          output,
                                          gt)
